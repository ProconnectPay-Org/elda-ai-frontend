...
export const ResumeStep1Schema = z.object({
  ...
  export const ResumeStep3Schema = z.object({
    ...
    tertiaryInstitutionAttended: z.string().nonempty("Tertiary institution attended is required"),
    city: z.string().nonempty("City is required"),
    state: z.string().nonempty("State is required"),
    course: z.string().nonempty("Course is required"),
    country: z.string().nonempty("Country is required"),
    startDate: z.string().nonempty("Start date is required").refine(date => {
      const parsedDate = Date.parse(date);
      return !isNaN(parsedDate);
    }, {
      message: "Invalid date format"
    }),
    endDate: z.string().nonempty("End date is required").refine(date => {
      const parsedDate = Date.parse(date);
      return !isNaN(parsedDate);
    }, {
      message: "Invalid date format"
  ...
  export const ResumeStep5Schema = z.object({
    selectField: z.string().nonempty("Selection is required"),
  });